...
import { PropertyCreateType } from '@airbnb/schema';
import { AnimatePresence, motion } from '@airbnb/ui/components';
import useOnClickOutside from '@airbnb/ui/hooks/use-onclick-outside';
import { useEffect, useRef, useState } from 'react';

const regions: {
    name: string;
    key: PropertyCreateType['region'] | 'all';
    image: string;
}[] = [
    {
    ...
    {
        image: 'https://a0.muscache.com/im/pictures/cd9f2bf0-eefc-4980-b7cb-9c8ca3dae883.jpg?im_w=320',
        key: 'canada',
        name: 'Canada',
    },
    {
    ...
    {
        image: 'https://a0.muscache.com/im/pictures/dbb2b5ef-2efe-4099-81ac-c7b957f384ed.jpg?im_w=320',
        key: 'united_kingdom',
        name: 'United Kingdom',
    },
    {
...
const Navbar = () => {
    ...
    const [selectedSearchArea, setSelectedSearchArea] = useState<'where' | 'checkin' | 'checkout' | 'who' | null>(
        'where'
    );

    useEffect(() => {
        if (openSearch) {
            setSelectedSearchArea('where');
        }
    }, [openSearch]);
    ...
    return (
        <>
            {openSearch ? <div className="bg-dark-gray/30 fixed z-40 h-screen w-screen transition-all"></div> : null}

            <header onClick={() => setOpenSearch(true)} ref={ref} className="sticky top-0 z-50 bg-white">
                <nav className="border-gray  relative flex h-20 items-center border-b border-opacity-50">
                    <motion.div
                        initial={false}
                        animate={{ height: openSearch ? '80px' : '0' }}
                        className="absolute top-full flex w-full  bg-white "
                    >
                        <motion.div
                            ref={searchRef}
                            initial={false}
                            animate={{
                                scale: openSearch ? 1 : 0.9,
                                opacity: openSearch ? 1 : 0,
                                y: openSearch ? 0 : -30,
                                scaleX: openSearch ? 1 : 0.5,
                            }}
                            className={`mx-auto flex h-16  w-full max-w-[848px] rounded-full border border-[#ddd] ${
                                selectedSearchArea ? 'bg-[#ebebeb]' : 'bg-white'
                            }`}
                        >
                            <div
                                onClick={() => setSelectedSearchArea('where')}
                                className={`relative flex h-full w-4/12  cursor-pointer select-none flex-col justify-center rounded-full px-8  ${
                                    selectedSearchArea === 'where'
                                        ? 'scale-[1.03] bg-white shadow-2xl '
                                        : 'hover:bg-black/10'
                                }`}
                            >
                                <h4 className="text-dark-gray mb-0.5 text-xs font-extrabold">Where</h4>
                                <p className="text-sm font-normal text-gray-500">Search destination</p>
                                {selectedSearchArea === 'where' ? (
                                    <div className="absolute left-2  top-[75px] w-[478px] max-w-3xl rounded-[30px] border border-gray-200 bg-white px-8 py-9">
                                        <h4 className="text-dark-gray  mb-8 text-sm font-bold">Search by region</h4>
                                        <div className="grid grid-cols-3 gap-x-4 gap-y-8 ">
                                            {regions.map((region) => (
                                                <div key={region.key} className={`flex cursor-pointer flex-col gap-2`}>
                                                    <div className="h-[122px] overflow-hidden rounded-xl border border-gray-200 hover:border-gray-500">
                            ...
                            <div
                                onClick={() => setSelectedSearchArea('checkin')}
                                className={`flex h-full w-2/12   cursor-pointer select-none flex-col justify-center rounded-full px-8  ${
                                    selectedSearchArea === 'checkin'
                                        ? 'scale-[1.03] bg-white shadow-2xl '
                                        : 'hover:bg-black/10'
                                }`}
                            >
                                <h4 className="text-dark-gray mb-0.5 text-xs font-extrabold">Check in</h4>
                                <p className="text-sm font-normal text-gray-500">Add dates</p>
                            </div>
                            <div
                                onClick={() => setSelectedSearchArea('checkout')}
                                className={`flex h-full w-2/12   cursor-pointer select-none flex-col justify-center rounded-full px-8  ${
                                    selectedSearchArea === 'checkout'
                                        ? 'scale-[1.03] bg-white shadow-2xl '
                                        : 'hover:bg-black/10'
                                }`}
                            >
                                <h4 className="text-dark-gray mb-0.5 text-xs font-extrabold">Check out</h4>
                                <p className="text-sm font-normal text-gray-500">Add dates</p>
                            </div>
                            <div
                                onClick={() => setSelectedSearchArea('who')}
                                className={`flex h-full w-4/12 cursor-pointer select-none  items-center justify-between rounded-full pl-5 pr-3  ${
                                    selectedSearchArea === 'who'
                                        ? 'scale-[1.03] bg-white shadow-2xl '
                                        : ' hover:bg-black/10'
                                }`}
                            >
                                <div className="flex h-full flex-col justify-center ">
                                    <h4 className="text-dark-gray mb-0.5 text-xs font-extrabold">Who</h4>
                                    <p className="text-sm font-normal text-gray-500">Add Guest</p>
                                </div>
                                <motion.button
                                    animate={{
                                        width: selectedSearchArea ? '110px' : '48px',
                                    }}
                                    className="flex h-12 items-center justify-center gap-2 rounded-full bg-[#ff385c]"
                                >
                                    <svg
                                        ...
                                        role="presentation"
                                        focusable="false"
                                        className="block  h-4 w-4 fill-none  "
                                        strokeWidth={4}
                                        stroke="white"
                                    >
                    ...
                    <div className="container mx-auto">
                        <div className="flex w-full">
                            <div className="flex w-3/12 items-center">
                                <svg className="text-[#ff385c]" width="102" height="32">
                                    <path
                                ...
                                <AnimatePresence initial={false}>
                                    {openSearch ? (
                                        <motion.div
                                            ...
                                            className=""
                                        >
                                            <div className="text-dark-gray flex h-10 flex-nowrap items-center  gap-7 text-base  font-normal">
                                                <p className="relative flex h-full items-center ">
                                                    <span className="bg-dark-gray absolute bottom-0 h-0.5 w-full"></span>
                                                    Stays
                                                </p>
                                                <p>Experiences</p>
                                        ...
                                        <motion.div
                                            key="search"
                                            initial={{ scale: 1.2, opacity: 0, y: 50, position: 'absolute' }}
                                            animate={{
                                                opacity: 1,
                                                scale: 1,
                                                y: 0,
                                            ...
                                            className="i1box flex h-12 items-center overflow-hidden"
                                        >
                                            <div className="flex flex-nowrap items-center gap-4">
                                                <button className="text-dark-gray text-sm font-semibold">
                                                    Anywhere
                                                </button>
                                                <span></span>
                                                <button className="text-dark-gray text-sm font-semibold">
                                                ...
                                                <span></span>
                                                <button className="text-light-gray flex items-center gap-[14px] text-sm font-medium">
                                                    Add guests
                                                    <div className="g-search flex h-8 w-8 items-center justify-center rounded-full bg-[#ff385c] text-white">
                                                        <svg
                                                            className="block h-3 w-3 overflow-auto fill-none stroke-current "
                            ...
                            <div className="flex w-3/12 shrink-0 items-center justify-end">
                                <div className="flex items-center gap-5">
                                    <button className="text-dark-gray rounded-full p-3 text-sm font-medium hover:bg-gray-200">
                                        Airbnb your home
                                    </button>
                                    <button>
                                        <svg
                                    ...
                                    <div className="flex items-center gap-4 rounded-full border px-3 py-1">
                                        ...
                                        <div className="text-light-gray">
                                            <svg
                                                className="block h-[30px] w-[30px] fill-current"
                                                xmlns="http://www.w3.org/2000/svg"
                                                viewBox="0 0 32 32"
                                                aria-hidden="true"
                                                role="presentation"
